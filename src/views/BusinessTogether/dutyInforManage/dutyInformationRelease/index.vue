<!-- 值班信息发布 -->
<template>
  <normal-layer :search-number="itemsDatas.length" title-name="值班信息发布列表" title-need-handle>
    ...
    <detail-dialog v-model="showDetailDialog" dialog-title="值班信息详情" />
...
export default {
  ...
  data() {
    return {
      loading: false,
      itemsDatas: [
        { label: '发布部门', prop: 'xxx', type: 'input' },
  ...
  methods: {
    addEvent() {
      this.dialogTitle = '值班信息新增'
      this.showEditDialog = true
    },
    editClick(row) {
      this.dialogTitle = '值班信息编辑'
      this.showEditDialog = true
    },
    detailsClick(row) {
