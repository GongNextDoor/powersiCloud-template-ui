<!-- 试卷信息维护 -->
<template>
  <normal-layer :search-number="5" title-name="试卷信息维护列表" title-need-handle>
    ...
    <detail-dialog v-model="showDetailDialog" dialog-title="试卷信息详情" />
...
export default {
  ...
  data() {
    return {
      ...
      itemsDatas: [
        ...
        { label: '试卷名称', prop: 'xxx', type: 'input' }
      ],
      queryForm: {
        xxx: '',
        dateRange: []
  ...
  methods: {
    addEvent() {
      this.dialogTitle = '试卷信息新增'
      this.showEditDialog = true
    },
    editClick(row) {
      this.dialogTitle = '试卷信息编辑'
      this.showEditDialog = true
    },
    detailsClick(row) {
